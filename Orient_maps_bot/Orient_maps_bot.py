...
def start(m):
    ...
    bot.send_message(m.chat.id, welcome_message, reply_markup=welcome_keyboard())
...
def callback_worker(call):
    if call.data == "upload_map":
        ...
        bot.register_next_step_handler(call.message, upload_map)
    ...
    if records[-1][2] == 'document':
        bot.send_document(message.chat.id, records[-1][5],
                          caption='Дата: ' + str(records[-1][3]) + '\n' + records[-1][4] + '\n' +
                                  'Карта лежит в файле с буквой "' + records[-1][6] +
...
def get_tags(message):  # получаем хэштэги
    ...
    try:
        ...
        bot.send_message(message.from_user.id,
                         "Укажи *первую букву* названия карты\n"
...
def add_file_type(message):
    ...
    try:
        ...
        log_db('add_file_type', message.from_user.id, str(e))
        bot.reply_to(message, 'Что-то пошло не так. Придётся начинать сначала')
...
def add_file(message):
    ...
    try:
        ...
        log_db('add_file', message.from_user.id, str(e))
        bot.reply_to(message, 'Что-то пошло не так. Придётся начинать сначала')
